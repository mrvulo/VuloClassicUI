-- Warlock DoT set (class data for the shared DoT tracker in Modules/ClassTools).
--
-- meta.desc is handed over as plain English: it IS the locale key, and the host
-- translates it when it draws the page. Resolving it here would happen at file
-- load, before the saved language override is applied.
local _, ns = ...
...
csMod:RegisterDotSet("WARLOCK", {
    ...
    desc = "|cffaaaaaaTracks your Warlock DoTs (Corruption, Curse of Agony, Unstable Affliction, Siphon Life, Immolate, Curse of Doom) on the target, with the same recast-snapshot readout as the Priest tracker.|r",
})
