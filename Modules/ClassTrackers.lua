-- VuloClassicUI / Modules / ClassTrackers: the two on-screen trackers of the
-- "Class Specific" page -- the Vampiric Touch mana counter and the DoT tracker.
--
-- Split out of Modules/ClassTools.lua, which is the page HOST: registration,
-- tabs, profile plumbing and options dispatch. Everything that owns a frame,
-- an event or a pixel lives here instead, so the host stays readable.
--
-- The two trackers stay in ONE file, and NOT in Modules/Classes/Priest.lua,
-- because they share a single COMBAT_LOG_EVENT_UNFILTERED handler (onCombatLog
-- below) that destructures the event once and feeds both. Two files would mean
-- two subscribers to the hottest event in the game, or a fan-out call per
-- event. The coupling is the optimisation; keep them together.
--
-- Loads after the host (it reads mod.dotSets) and before Modules/Classes/, so
-- a class file that registers a DoT set finds the registry ready.
--
...
local mod = ns.modules and ns.modules.vtmanadisplay
if not mod then return end

-- classToken -> { dot defs }, filled by Modules/Classes/ through the host.
local DOT_SETS = mod.dotSets
local dotDefs  = {}   -- active set, chosen by class in Enable
...
local function trackersEnable(class)
...
mod.trackers = {
    Enable           = trackersEnable,
    Disable          = trackersDisable,
    PriestOptions    = priestOptions,
    AppendDotOptions = appendDotTracker,
}
