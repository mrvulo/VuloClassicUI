-- VuloClassicUI / UI / Setup: the first-time setup, three steps in a modal.
--
...
-- Reachable again through /vcui setup and a button under Global Settings.
local _, ns = ...
local L = ns.L
ns.UI = ns.UI or {}
local UI = ns.UI
...
local function buildStep3(page)
    ...
    hint:SetText(L["You can run this again any time with /vcui setup or from Global Settings."])
    local d = ns.COLORS.textDim
...
local function ensureShell()
    ...
    host = CreateFrame("Frame", "VCUI_SetupHost", UIParent)
    ...
    table.insert(_G.UISpecialFrames, "VCUI_SetupHost")
    ...
    w.title:SetText(L["Set up VuloClassicUI"])
